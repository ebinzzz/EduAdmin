...
@section('breadcrumb', 'Home / Administration / Class Management / Edit Class')
...
<div class="dashboard-wrapper">
    <div class="main-hero">
        <div class="hero-inner">
            ...
            <p class="hero-description">Update the class details below</p>
        </div>
    </div>
...
@endif
    ...
    <div class="form-container">
        <form id="createClassForm" method="POST" action="{{ route('classes.update', $class->id) }}" class="class-form">
            @csrf
            @method('PUT')
            ...
            <div class="form-section">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <div class="select-wrapper">
                            ...
                            <select class="form-select @error('name') is-invalid @enderror" id="class_name" name="name" required>
                                <option value="">Select Class</option>
                                <option value="LKG" {{ old('name', $class->name) == 'LKG' ? 'selected' : '' }}>LKG</option>
                                <option value="UKG" {{ old('name', $class->name) == 'UKG' ? 'selected' : '' }}>UKG</option>
                                @for($i = 1; $i <= 12; $i++)
                                    <option value="{{ $i }}" {{ old('name', $class->name) == $i ? 'selected' : '' }}>Class {{ $i }}</option>
                                @endfor
                            </select>
                        </div>
                        ...
                        <div class="select-wrapper">
                            ...
                            <select class="form-select @error('section') is-invalid @enderror" id="section" name="section" required>
                                ...
                                @foreach(range('A', 'G') as $letter)
                                    <option value="{{ $letter }}" {{ old('section', $class->section) == $letter ? 'selected' : '' }}>Section {{ $letter }}</option>
                                @endforeach
                            </select>
                        </div>
                        @error('section')<span class="error-text">{{ $message }}</span>@enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        @error('academic_year')<span class="error-text">{{ $message }}</span>@enderror
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        @error('class_teacher_id')<span class="error-text">{{ $message }}</span>@enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-wrapper">
                            ...
                            <input type="number" class="form-input @error('student_capacity') is-invalid @enderror" id="student_capacity" name="student_capacity" value="{{ old('student_capacity', $class->student_capacity) }}" min="1" max="100" placeholder="40">
                        </div>
                        @error('student_capacity')<span class="error-text">{{ $message }}</span>@enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-wrapper">
                            ...
                            <input type="text" class="form-input @error('room_number') is-invalid @enderror" id="room_number" name="room_number" value="{{ old('room_number', $class->room_number) }}" placeholder="e.g., R-101">
                        </div>
                        @error('room_number')<span class="error-text">{{ $message }}</span>@enderror
                    </div>
                </div>
            </div>
            ...
            <div class="form-section">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <div class="input-wrapper">
                            ...
                            <input type="text" class="form-input @error('building') is-invalid @enderror" id="building" name="building" value="{{ old('building', $class->building) }}" placeholder="e.g., Main Block">
                        </div>
                        @error('building')<span class="error-text">{{ $message }}</span>@enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="select-wrapper">
                            ...
                            <select class="form-select @error('floor_number') is-invalid @enderror" id="floor_number" name="floor_number">
                                <option value="">Select Floor</option>
                                <option value="0" {{ old('floor_number', $class->floor_number) == '0' ? 'selected' : '' }}>Ground Floor</option>
                                @for($i = 1; $i <= 4; $i++)
                                    <option value="{{ $i }}" {{ old('floor_number', $class->floor_number) == $i ? 'selected' : '' }}>
                                        {{ $i }}{{ $i == 1 ? 'st' : ($i == 2 ? 'nd' : ($i == 3 ? 'rd' : 'th') ) }} Floor
                                    </option>
                                @endfor
                            </select>
                        </div>
                        @error('floor_number')<span class="error-text">{{ $message }}</span>@enderror
                    </div>
                ...
                <div class="form-row">
                    <div class="form-group full-width">
                        ...
                        <textarea class="form-textarea @error('description') is-invalid @enderror" id="description" name="description" rows="4" placeholder="Optional description about the class">{{ old('description', $class->description) }}</textarea>
            ...
            <div class="form-actions">
                <a href="{{ route('class-management.index') }}" class="btn btn-cancel">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        if (!isValid) {
            ...
            submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Updating Class...';
        }
    });
});
</script>
