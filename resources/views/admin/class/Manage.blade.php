...
#classCardContainer {
    ...
    scroll-behavior: smooth;  /* Smooth scroll for horizontal overflow */
}
    ...
    <div class="main-content-card">
        <div class="content-header">
            ...
            <div class="action-buttons">
                    <a href="{{ route('classes.create') }}" class="btn-enhanced btn-primary-enhanced">
        ...
        <ul class="nav-tabs-custom">
            <li class="nav-tab-custom">
                <a href="#" class="nav-link-custom active" onclick="switchTab(event, 'classes-tab')">
                    <i class="fas fa-chalkboard"></i>
                    Classes Overview
                </a>
            </li>
            <li class="nav-tab-custom">
                <a href="#" class="nav-link-custom" onclick="switchTab(event, 'amenities-tab')">
                    <i class="fas fa-chair"></i>
                    Amenities Management
                </a>
            </li>
            <li class="nav-tab-custom">
                <a href="#" class="nav-link-custom" onclick="switchTab(event, 'maintenance-tab')">
                    <i class="fas fa-tools"></i>
                    Maintenance Records
                </a>
            </li>
            <li class="nav-tab-custom">
                <a href="#" class="nav-link-custom" onclick="switchTab(event, 'reports-tab')">
        ...
        <div class="tab-content-custom">
            ...
            <div id="classes-tab" class="tab-pane-custom active">
                ...
                <div class="filters-section">
                    ...
                    <div class="filter-group">
                        ...
                        <input type="text" class="form-control-enhanced" id="searchFilter" placeholder="Search classes..." oninput="filterClasses()">
                    </div>
                </div>
                ...
                <div class="classes-grid" id="classesGrid">
                    <div id="classCardContainer">
                            ...
                            @php
                                 $className = strtoupper($class->name); // Just the name (e.g., LKG, 1, 10)
                                $displayName = $class->name . ' - ' . $class->section;
                                // Multiple ways to check for teacher assignment - try these alternatives:
                                
                                // Option 1: Check if classTeacher exists and has required fields
                                $teacherName = ($class->classTeacher && $class->classTeacher->first_name) 
                                    ? trim($class->classTeacher->first_name . ' ' . $class->classTeacher->last_name)
                                    : 'Not Assigned';
                                
                                // Option 2: Use null coalescing operator (uncomment if Option 1 doesn't work)
                                // $teacherName = $class->classTeacher->first_name ?? 'Not Assigned';
                                // if ($teacherName !== 'Not Assigned') {
                                //     $teacherName = trim($class->classTeacher->first_name . ' ' . ($class->classTeacher->last_name ?? ''));
                                // }
                                
                                // Option 3: Check relationship name variations (uncomment if needed)
                                // $teacherName = $class->class_teacher 
                                //     ? trim($class->class_teacher->first_name . ' ' . $class->class_teacher->last_name)
                                //     : 'Not Assigned';
                                
                                // Option 4: Check for teacher_id field (uncomment if needed)
                                // $teacherName = $class->teacher_id && $class->teacher 
                                ...
                                $floor = is_null($class->floor_number) 
                                    ? 'Not Assigned' 
                                    : ($class->floor_number == 0 ? 'Ground Floor' : $class->floor_number . 'st Floor');
                                ...
                                // Determine grade type based on class name
                                    $grade=$class->grade_type;
                                     
                            @endphp

                            <div class="class-card" data-grade-type="{{ $grade }}" data-academic-year="{{ $class->academic_year }}" data-status="{{ $status }}">
                                <div class="class-card-header">
                                        @if ($className === 'LKG' || $className === 'UKG')
                                        <h3 class="class-name">{{ $className }} - {{ $class->section }}</h3>
                                    @else
                                        <h3 class="class-name">Class {{ $displayName }}</h3>
                                    @endif
            ...
            <!-- Amenities Tab -->
            <div id="amenities-tab" class="tab-pane-custom" style="display: none;">
            ...
            <!-- Maintenance Tab -->
            <div id="maintenance-tab" class="tab-pane-custom" style="display: none;">
                <div style="padding: 1.5rem;">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <!-- Search Bar -->
                    <div class="mb-4">
                        <input type="text" class="form-control-enhanced" placeholder="Search maintenance records..." style="max-width: 400px;" id="maintenanceSearch">
                    </div>
            ...
            <!-- Reports Tab -->
            <div id="reports-tab" class="tab-pane-custom" style="display: none;">
                <div style="padding: 1.5rem;">
...
<script>
    ...
    function switchTab(event, tabId) {
        ...
        const clickedLink = event ? event.target.closest('.nav-link-custom') : document.querySelector(`[onclick*="${tabId}"]`);
        if (clickedLink) {
    ...
    // Fixed: Filter classes function
    function filterClasses() {
        const gradeType = document.getElementById('gradeTypeFilter').value.toLowerCase();
        ...
        classCards.forEach(card => {
            ...
            const gradeMatch = !gradeType || cardGradeType === gradeType;
            const yearMatch = !academicYear || cardAcademicYear === academicYear;
            const statusMatch = !status || cardStatus === status;
            const searchMatch = !search || cardText.includes(search);
            
            const shouldShow = gradeMatch && yearMatch && statusMatch && searchMatch;
        ...
        if (visibleCount === 0) {
            if (!noResultsMsg) {
                ...
                noResultsMsg.innerHTML = '<h5>No classes found matching your criteria</h5>';
                document.getElementById('classCardContainer').appendChild(noResultsMsg);
            }
    ...
    // Fixed: Initialize maintenance search
    function initializeMaintenanceSearch() {
    ...
    function openScheduleMaintenanceModal() {
        showAlert('Schedule Maintenance Modal would open here', 'info');
    }

    function viewAmenities(classId) {
        const classData = classes.find(c => c.id === classId);
        if (classData) {
            showAlert(`View Amenities for: ${classData.display_name}`, 'info');
        }
    }
    ...
    function manageStudents(classId) {
        const classData = classes.find(c => c.id === classId);
        if (classData) {
            showAlert(`Manage Students for: ${classData.display_name}`, 'info');
        }
    }

    // Amenity management functions
    function editAmenity(amenityId) {
        showAlert(`Edit Amenity ID: ${amenityId}`, 'info');
    }

    function scheduleMaintenance(amenityId) {
        showAlert(`Schedule Maintenance for Amenity ID: ${amenityId}`, 'info');
    }

    // Maintenance functions
    function viewMaintenanceDetails(recordId) {
        showAlert(`View Maintenance Details for Record ID: ${recordId}`, 'info');
    }

    function updateMaintenanceStatus(recordId) {
        showAlert(`Update Status for Maintenance Record ID: ${recordId}`, 'success');
    }

    function deleteMaintenanceRecord(recordId) {
        if (confirm('Are you sure you want to delete this maintenance record?')) {
            showAlert(`Maintenance Record ID: ${recordId} deleted successfully`, 'success');
        }
    }
